...
test.describe('CRM customer - Positive case', () => {
  test('TC_CUST_02- tạo Customer (Nhập tất cả trường)', async ({ page }) => {
    ...
    await expect(page.locator("#alert_float_1")).toContainText(
      "Customer added successfully."
    );
    const currencyValue = await currencyField.textContent();
    expect(currencyValue).toContain("USD");
    expect(
      await page.locator("button[data-id='country']").textContent()
    ).toContain("Vietnam");
    expect(await page.locator("#vat").inputValue()).toContain(
      information.vatNumber
    );
    expect(await page.locator("#phonenumber").inputValue()).toContain(
      information.phoneNumber
    );
    expect(await page.locator("#website").inputValue()).toContain(
      information.websiteUrl
    );
    expect(await page.locator("#address").inputValue()).toContain(
      information.address
    );
    expect(await page.locator("#city").inputValue()).toContain(
      information.city
    );
    expect(await page.locator("#state").inputValue()).toContain(
      information.state
    );
    expect(await page.locator("#zip").inputValue()).toContain(
      information.zipCode
    );
  });
});
test.describe("CRM customer - Kiểm thử chức năng UI", () => {
  test('TC_CUST_04- Kiểm tra "Same as Customer Info" (Billing)', async ({
    ...
    await test.step("2. Nhập các thông tin khác", async () => {
      ...
      await page
        .locator(".dropdown-menu")
        .getByRole("option", { name: information.currency })
        .click();
      //country
    ...
    await test.step("5. Verify thông tin Billing được copy đúng", async () => {
      await expect(page.locator("#billing_street")).toHaveValue(
        information.address
      );
      ...
      await expect(page.locator("#billing_zip")).toHaveValue(
        information.zipCode
      );
      expect(
        await page.locator("button[data-id='billing_country']").textContent()
      ).toContain(information.country);
    });
  });
  test('TC_CUST_05- Kiểm tra "Same as Customer Info" (Billing)', async ({
    page,
  }) => {
    const information = infomation();
    await loginCRMAndNavigateToCustomer(page, "Customers");
    ...
    await test.step("5. Verify thông tin Shipping Address is copied from Billing ", async () => {
      await expect(page.locator("#shipping_street")).toHaveValue(
        information.address
      );
      await expect(page.locator("#shipping_city")).toHaveValue(
        information.city
      );
      await expect(page.locator("#shipping_state")).toHaveValue(
        information.state
      );
      await expect(page.locator("#shipping_zip")).toHaveValue(
        information.zipCode
      );
      expect(
        await page.locator("button[data-id='shipping_country']").textContent()
      ).toContain(information.country);
    });
  });
});
test.describe("CRM customer - Negative case", () => {
  ...
  test("TC_CUST_07- Cảnh báo Company đã tồn tại", async ({ page }) => {
    ...
    // có thể dùng page.goto('https://crm.anhtester.com/admin/clients/client') để back lại nhanh tạo customer
    await test.step("4. Click add Quick create", async () => {
      await page
        .getByRole("listitem", { name: "Quick Create" })
        .getByRole("link")
        .click();
      await page
        .locator('[class*="dropdown-menu"]')
        .filter({ hasText: "Quick Create" })
        .getByRole("link", { name: "Customer" })
        .click();
    });

    await test.step("5. Nhập lại tên công ty đã tạo", async () => {
      await page.locator("#company").fill(information.companyName);
      //có thể dùng page.locaor().blur() để bỏ focus
      await page.keyboard.press("Tab");
      await expect(page.locator("#company_exists_info")).toContainText(
        `It looks that a customer with name ${information.companyName} already exists, if you still want to create the customer you can ignore this message.`
      );
    });
  });
});
