...
test('TC01. Register -> Login -> Lấy JWT token', async ({ request }) => {
  ...
  // body gửi lên khi đăng kí tạo 1 object user mới
  const newUser = {
...
test('TC06. Multipart form - upload file', async ({ request }) => {
  //   const token = await getToken(request);]]
  const fileContent1 = Buffer.from('Đây là nội dung file test upload1');// taoj ra file
  ...
  const listRes = await request.post('/public/test/echo-form', {
    multipart: {
      ...
      avatar2: {
        ...
        mimeType: 'image/jpeg',
        buffer: fileContent2, // nhớ chuyển file trong dạng buffer
      },
    },
  });
  const listBody = await listRes.json();
...
test('TC08. Multipart form - upload image', async ({ request }) => {
  //   const token = await getToken(request);]]
  const fileContent1 = Buffer.from('Đây là nội dung file test upload1');// taoj ra file
  const fileContent2 = Buffer.from('Đây là nội dung file test upload2');
