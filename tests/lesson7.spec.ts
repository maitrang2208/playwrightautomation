...
import { format, addDays, subDays, differenceInDays, parse, isValid, getMonth, getYear } from 'date-fns';
...
async function pickAntOptionByText(page: Page, text: string) {
  const option = page.locator('[class*="ant-select-dropdown"]').getByTitle(text);
...
async function selectDateDemo2(page: Page, ymd: string) {
  ...
  console.log(today);
  console.log(parsed);
  console.log(monthNames[getMonth(parsed)]);
  today.setHours(0, 0, 0, 0);
  if (parsed > today) {
    throw new Error('Khong the chon ngay tuong lai');
  }
  ...
  //tim ngay
  //const dayCell = page.locator('#dp2-table').getByRole('gridcell').locator(`[data-date='${ymd}']`);
  const dayCell = page.locator('#dp2-table').locator(`[data-date="${ymd}"]`);
  await dayCell.click();
